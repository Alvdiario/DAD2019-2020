					...
					if (res.succeeded()) {
						...
						for (Row row : resultSet) {
							result.add(JsonObject.mapFrom(new SensorValue(row.getInteger("idsensor_valor_mpu6050"),
									...
									row.getInteger("g_value_z"), row.getLong("timestamp"))));
						}
...
/*
	private void getUsuario(RoutingContext routingContext) {
		mySQLPool.query("SELECT * FROM dadproyecto.usuario WHERE idusuario = "
				+ routingContext.request().getParam("idusuario"), res -> {
					if (res.succeeded()) {
						...
						JsonArray result = new JsonArray();
						for (Row row : resultSet) {
							result.add(JsonObject.mapFrom(new Usuario(row.getInteger("idsensor_valor_mpu6050"),
									row.getString("idsensor")/*, row.getInteger("a_value_x"),
									row.getInteger("a_value_y"), row.getInteger("a_value_z"),
									row.getInteger("g_value_x"), row.getInteger("g_value_y"),
...
/*
	/*private void putValueForSensor(RoutingContext routingContext) {
		...
		mySQLPool.preparedQuery("INSERT INTO sensor_value (idsensor, value, accuracy, timestamp) VALUES (?,?,?,?)",
				Tuple.of(sensorValue.getIdsensor(), sensorValue.getValue(), sensorValue.getAccuracy(),
